...
func TestManagedBackupsAreScopedToVaultRoot(t *testing.T) {
	...
	newBackupPath := filepath.Join(root, "LW-Manager-Backup-2026.zip")
	newRollbackPath := filepath.Join(root, "LW-Manager-Rollback-2026.zip")
	for _, path := range []string{backupPath, rollbackPath, newBackupPath, newRollbackPath, filepath.Join(root, "anderes.zip")} {
